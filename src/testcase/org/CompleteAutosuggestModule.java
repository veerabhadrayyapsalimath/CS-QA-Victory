...
public class CompleteAutosuggestModule extends BaseTest {
  ...
  //test case to check selected infield is working
	
	@Test(priority=12)
	public void infield()
	{
		String y = Excel.getCellCalue(EXCEL_DATA, "Sheet3", 1, 4);
		log.info(y);
		driver.findElement(By.xpath(AutoUtility.getProperty(CONFIG_PROPERTY2, "searchbox"))).sendKeys(y);
		  List <WebElement> allInfields = driver.findElements(By.className((AutoUtility.getProperty(CONFIG_PROPERTY2, "insuggestion"))));
	
	 int infieldNumber    =    allInfields.size();
    System.out.println("Test case to check if infields are present in site");
    System.out.println("Infields are present. number of infields are: "+infieldNumber);
    
    
    if(infieldNumber>0)
    {
         //test case to check selected infield is working
                System.out.println("Test case to check if infield is working");
        String x = driver.findElement(By.xpath((AutoUtility.getProperty(CONFIG_PROPERTY2, "firstInsuggestion")))).getAttribute("data-filtervalue");
        System.out.println("Infield name : "+x);
        
        //click on infield
        driver.findElement(By.xpath((AutoUtility.getProperty(CONFIG_PROPERTY2, "firstInsuggestion")))).click();
        
        
         String cc=driver.getCurrentUrl();
       
         if(cc.contains(x))
         {
             System.out.println("Pass: selected infield is working on click");
         }
         else
         {
             System.out.println("fail: selected infield is not working on click");
         }
    
    }
    else //case where infield is not present
    {
        System.out.println("infields are not present");
    }
    driver.quit();
...
//-------------------------------------------------------------------------------------------------------------------
	...
	//test case to search infield using search icon
	@Test(priority=13)
	public void infield2() throws InterruptedException
	{
		String y = Excel.getCellCalue(EXCEL_DATA, "Sheet3", 1, 4);
		log.info(y);
		driver.findElement(By.xpath(AutoUtility.getProperty(CONFIG_PROPERTY2, "searchbox"))).sendKeys(y);
    ...
    {
        //test case to search infield using search icon   
        String preURL1=driver.getCurrentUrl();
                Thread.sleep(2000);
     List <WebElement> allInfields2 = driver.findElements(By.className((AutoUtility.getProperty(CONFIG_PROPERTY2, "insuggestion"))));
     Actions act = new Actions(driver);
     act.moveToElement(allInfields2.get(0)).perform();
     int sizeofinfield2 = allInfields2.size();
     System.out.println("number of infields are: "+sizeofinfield2);
     driver.findElement(By.xpath(AutoUtility.getProperty(CONFIG_PROPERTY2, "searchicon"))).click();
     Thread.sleep(2000);
     System.out.println("Clicking on search icon after selecting infield");
     String preURL2=driver.getCurrentUrl();
     if(preURL1==preURL2)
     System.out.println("test case failed: selecting infield via search icon is not working");
     else
         System.out.println("Test case passed: selecting infield via search icon is working");
...
//-------------------------------------------------------------------------------------------------------------	
	...
	@Test(priority=14)
	public void infield3() throws InterruptedException
	{
		String y = Excel.getCellCalue(EXCEL_DATA, "Sheet3", 1, 4);
		log.info(y);
		driver.findElement(By.xpath(AutoUtility.getProperty(CONFIG_PROPERTY2, "searchbox"))).sendKeys(y);
    ...
    if(infieldNumber>0)
    {
         String preURL3=driver.getCurrentUrl();
    
         Thread.sleep(2000);
         List <WebElement> allInfields3 = driver.findElements(By.className("unbxd-as-insuggestion"));
         Actions act = new Actions(driver);
         act.moveToElement(allInfields3.get(0)).perform();
         driver.findElement(By.xpath(AutoUtility.getProperty(CONFIG_PROPERTY2, "searchicon"))).sendKeys(Keys.ENTER);
         String preURL4=driver.getCurrentUrl();
         if(preURL3==preURL4)
             System.out.println("test case failed: selecting infield via Enter key is not working");
             else
                 System.out.println("Test case passed: selecting infield via Enter key is working");
